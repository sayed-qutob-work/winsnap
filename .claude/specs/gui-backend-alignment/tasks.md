...
- [ ] 4. Update `gui.py` Qt widgets for manifest ordering and honest reporting
...
- [ ] 4.2 Add "Verify after restore" checkbox to `RestoreView`
...
- [ ] 4.3 Extend `ResultsView` for four-status grouping, reasons, per-item detail, and verify results
...
- [ ] 5. Rewrite `ExportWorker` and `RestoreWorker` as thin backend adapters
...
- [ ] 5.2 Rewrite `RestoreWorker._do_restore()`
...
- [ ] 5.3 Add worker integration tests for the rewritten adapters
...
- [ ] 6. Wire `MainWindow` export collision pre-check
  - Depends on Task 5 (needs `ExportConfig.force` and the rewritten `ExportWorker`).
  - _Requirements: 9.1, 9.2, 9.3_

- [ ] 6.1 Add collision pre-check to `try_start_export()`
...
- [ ] 7.1 Rewrite `tests/test_prop_restore_outcome.py`
...
- [ ] 7.2 Update `tests/test_prop_export_outcome.py`
  - Update assertions for `classify_export_outcome`'s `PASSED` → `MATCHED` rename; function itself is retained.
  - _Requirements: 1.5, 11.6_

- [ ] 7.3 Rewrite `tests/test_prop_version_verdict.py` and `tests/test_prop_version_info_msg.py`
...
- [ ] 7.4 Rewrite `tests/test_prop_module_resolution.py`
...
- [ ] 7.5 Repoint `tests/test_snapshot_naming.py` and `tests/test_prop_snapshot_name.py`
...
- [ ] 7.6 Rewrite `tests/test_restore_worker.py` monkeypatch targets
...
- [ ] 7.9 Update `tests/test_widget_states.py`
